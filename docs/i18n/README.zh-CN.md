...
项目面向 Windows、macOS 和 Linux。当前本地已验证 Windows x64 成品。macOS Intel、macOS Apple Silicon 和 Linux 成品由 GitHub Actions 在对应系统 runner 上构建。ARM64 是发布策略的一部分，其中 macOS ARM64 使用原生 runner，其他 ARM64 平台保留源码构建和后续 runner 扩展能力。

## 安装

普通用户应从 GitHub Releases 下载成品。Windows 用户可以选择安装包或便携版 zip；macOS 用户使用 DMG；Linux 用户使用 AppImage 或 Debian 包。

运行前需要 Node.js 18 或更高版本，并确保 `node` 在 `PATH` 中。Claude Code 如果没有安装，应用会显示安装引导。
...
`0.1.0` 已整理为可以发布到 GitHub 的版本。Node.js 仍是明确的运行依赖。代码签名和自动更新还没有启用。
